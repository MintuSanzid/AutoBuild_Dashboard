...
using Dashboard_HR.Repository.Dashboards;
...
namespace Dashboard_HR.Data.Handler
{
    public class BusinessplanHandler
    {
        ...
        private List<BusinessplanModel> _aBusinessplanData;
        public IGenRepository<DataTable> ARepository; 
        ...
        public List<BusinessplanModel> GetDashboardFindByCompanyData(BusinessInfo aBusinessInfo)
        {
            try
            {
        ...
        private List<BusinessplanModel> ListGenerateGetDashboardFindByCompanyData(DataSet aDataSet)
        {
            _aBusinessplanData = new List<BusinessplanModel>();
        ...
        private void GenerateDataTableToList(DataSet aDataSet)
        {
            ...
            {
                var aData = new BusinessplanModel
                {
        ...
        public List<BusinessplanModel> GetBpCapacityData(BusinessInfo aBusinessInfo)
        {
            try
            {
        ...
        private List<BusinessplanModel> ListGenerateGetBpCapacityData(DataSet aDataSet)
        {
            _aBusinessplanData = new List<BusinessplanModel>();
            GenerateDataTableToList(aDataSet);
            return _aBusinessplanData;
        ...
        /// Line KPI DataTable 
        /// </summary>
        public DataTable GetLineKpiDataList(BusinessInfo aInfo)
        {
            try
            {
                var companycode = aInfo.CompanyCode;
                var cCode = companycode.Replace(",", "','");
                ARepository = new BusinessplanRepository<DataTable>();
                var data = ARepository.GetAll(cCode, aInfo.FromDate, aInfo.ToDate);
                return data;
            }
            catch (Exception ex)
            {
