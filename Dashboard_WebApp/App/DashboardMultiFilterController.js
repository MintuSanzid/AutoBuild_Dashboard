...
appDashboard.controller("DashboardMultiFilterController", function ($scope, $rootScope, $http, $filter, $window, ngDialog) {
    ...
    $scope.multiFilterDLLAll = function () {
        $http.get("/DashboardMultiFilter/DashboardAllFilterDll").then(function (allFilters) {
    ...
    $scope.openingMultiFilter = function (employeeType) {
        ...
        var info = {
            ...
            LineCode: "01",
            ActivityCode: "01",
            CurrentDate: $filter("date")($scope.date, "MM/dd/yyyy") //"05/26/2017"
        };

        var config = { params: info, headers: { 'Accept': "application/json" } };
        $http.get("/DashboardMultiFilter/DashboardCompany", config).then(function (company) {
            if (company.data.length > 0 && company.status === 200) {
                ...
                //$http.get("/DashboardMultiFilter/DashboardDivision", config).then(function (division) {
                //    if (division.data.length > 0 && division.status === 200) {
                //        $scope.Divisions = division.data;
                //        $scope.DivisionsAll = division.data;
                //        $http.get("/DashboardMultiFilter/DashboardUnit", config).then(function (unit) {
        ...
        //$http.get("/DashboardMultiFilter/DashboardDivision", config).then(function (response) {
        //    if (response.data.length > 0 && response.status === 200) {
        //        $scope._divisions = response.data;
        //    }
        //});
        //$http.get("/DashboardMultiFilter/DashboardUnit", config).then(function (response) {
        //    if (response.data.length > 0 && response.status === 200) {
    ...
    $scope.filtedByCompanyWithAll = function () {
        ...
        info = {
            ...
            SubSectionCode: subsection.toString(),
            LineCode: line.toString(),
            ActivityCode: "01",
    ...
    $scope.openOnrollEmpDetails = function (codes, activitycode) {
        ...
        $http.get("/DashboardMultiFilter/DashboardAllocatedEmpListMultiFilter", config).then(function (response) {
            if (response.data.length > 0 && response.status === 200) {
        ...
        ngDialog.open({ template: "OnrollEmpList", controller: "DashboardMultiFilterController", className: "ngdialog-theme-default", scope: $scope });
