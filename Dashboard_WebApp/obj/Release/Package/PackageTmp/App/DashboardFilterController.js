...
appDashboard.controller("DashboardFilterController", function ($scope, $rootScope, $http, $filter, $window, ngDialog) {

    $scope.companyname = "Bitopi Group (BG)";
    $scope.Message = "";
    $http.get("/DashboardFilter/DashboardAllFilterDll").then(function (allFilters) {
    ...
    $scope.openingMultiFilter = function (employeeType) {
        ...
        $http.get("/DashboardFilter/DashboardCompany", config).then(function (company) {
            if (company.data.length > 0 && company.status === 200) {
                ...
                $http.get("/DashboardFilter/DashboardDivision", config).then(function (division) {
                    if (division.data.length > 0 && division.status === 200) {
                        $scope.Divisions = division.data;
                        $scope.DivisionsAll = division.data;
                        $http.get("/DashboardFilter/DashboardUnit", config).then(function (unit) {
                            if (unit.data.length > 0 && unit.status === 200) {
                                $scope.Units = unit.data;
                                $scope.UnitAll = unit.data;
        ...
        $http.get("/DashboardFilter/DashboardDivision", config).then(function (response) {
            if (response.data.length > 0 && response.status === 200) {
                $scope._divisions = response.data;
            }
        });
        $http.get("/DashboardFilter/DashboardUnit", config).then(function (response) {
            if (response.data.length > 0 && response.status === 200) {
    ...
    $scope.FiltedByCompanyWithAll = function () { 
        ...
        var info = {
            ...
            DivisionCode: divisionlist.toString(),
            UnitCode: unitlist.toString(),
            DepartmentCode: department.toString()
        };
        
        info.DepartmentCode="01";
        info.SectionCode="01";
        info.SubSectionCode="01";
        ...
        $http.get("/DashboardFilter/DashboardCompany", config).then(function (company) {
            if (company.data.length > 0 && company.status === 200) {
                ...
                $http.get("/DashboardFilter/DashboardDivision", config).then(function (division) {
                    if (division.data.length > 0 && division.status === 200) {
                        $scope.Divisions = division.data;
                        $scope.DivisionAll = division.data;
                        $http.get("/DashboardFilter/DashboardUnit", config).then(function (unit) {
                            if (unit.data.length > 0 && unit.status === 200) {
                                $scope.Units = unit.data;
                                $scope.UnitAll = unit.data;
    ...
    $scope.OpenngDiologValidation_Dept = function (event) {
        ...
        $http.get("/DashboardFilter/DashboardDepartment", config).then(function (dept) {
        ...
        ngDialog.open({ template: "Validation_DepartmentList", controller: "DashboardFilterController", className: "ngdialog-theme-default", scope: $scope });
    };
    ...
    $scope.OpenngDiologValidation = function () {
        ngDialog.open({ template: "Validation_Id", controller: "DashboardFilterController", className: "ngdialog-theme-default", scope: $scope });
    };
    ...
    $scope._openExcessList = function (event) {
        ...
        ngDialog.open({ template: "ExcessEmpList", controller: "DashboardFilterController", className: "ngdialog-theme-default", scope: $scope });
    ...
    $scope._openUnallocated = function (event) {
        ...
        ngDialog.open({ template: "UnallocatedEmpList_Table", controller: "DashboardFilterController", className: "ngdialog-theme-default", scope: $scope });
    ...
    $scope.ShowLineDetails = function (codes) {
        ...
        $http.get("/DashboardFilter/DashboardAllocatedEmpList", config).then(function (response) {
            if (response.data.length > 0 && response.status === 200) {
                $scope.employees = response.data;
            } else {
        ...
        ngDialog.open({ template: "AllocatedEmpList", controller: "DashboardFilterController", className: "ngdialog-theme-default", scope: $scope });
